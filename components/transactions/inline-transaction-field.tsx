    ...
    <form action={updateTransactionFieldAction} className={wrapperClassName} ref={formRef}>
      ...
      <span className={cn("inline-flex w-full items-center gap-1", kind === "kamas" && "w-auto", className)}>
        <input
          ...
          className={cn(
            "min-w-0 rounded-md border border-transparent bg-transparent py-1.5 font-medium text-slate-100 outline-none transition focus:border-mint focus:bg-panelSoft md:py-2",
            kind === "text" && "w-full",
...
export const InlinePackType = ({ id, field, value, ariaLabel, wrapperClassName }: InlinePackTypeProps) => {
  ...
  return (
    <form action={updateTransactionFieldAction} className={wrapperClassName} ref={formRef}>
      ...
      <select
        aria-label={ariaLabel}
        className="w-[3.8rem] rounded-md border border-line bg-panelSoft px-2 py-1.5 text-xs font-semibold text-slate-100 outline-none focus:border-mint md:w-[4.25rem] md:py-2 md:text-sm"
        defaultValue={value}
        name="value"
        onChange={() => {
          startTransition(() => {
...
export const InlineTransactionStatus = ({ id, value, wrapperClassName }: InlineTransactionStatusProps) => {
  ...
  return (
    <form action={updateTransactionFieldAction} className={wrapperClassName} ref={formRef}>
      ...
      <select
        aria-label="Statut"
        className="w-full rounded-md border border-line bg-panelSoft px-2 py-1.5 text-xs font-medium text-slate-100 outline-none focus:border-mint md:py-2 md:text-sm"
        defaultValue={value}
        name="value"
        onChange={() => {
          startTransition(() => {
